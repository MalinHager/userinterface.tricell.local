...
function getVirusImagesHTML(virusId) {
    ...
    html += `
        <form id="upload-image-form" action=""+config.webaddress+"http://localhost:3000/api/editvirusimage/newvirusimage/${virusId}" method="POST" enctype="multipart/form-data" style="display:none;">
        ...
        files.forEach(file => {
            ...
            html += `
                <div class="image-item" style="position:relative; text-align:center;">
                    ...
                    <a class="delete-image-btn" href=""+config.webaddress+"http://localhost:3000/api/editvirusimage/deletevirusimage/${virusId}/${imageNumber}">
                        &#x2715;
                    </a>
                </div>
            `;
