    ...
    if (canEdit(request))
    {
        ...
        async function sqlQuery()
        {
            ...
            {   
                ...
                if(canEdit(request))
                {
                    htmlOutput += "<div class=\"table-body-cell\"><a href=\""+config.webaddress+ "http://localhost:3000/api/activitylog/" + str_id + "\" style=\"color:red;text-decoration:none;\">D</a></div>\n"; // Gör till knapp
                }
                htmlOutput += "</div>\n";
            }  
    ...
    async function deleteSQLquery()
    {
        ...
        if(canEdit(request))  // check permissions first
        {
            ...
            if(result.length > 0)
            {
                ...
                response.write("<a href=\""+config.webaddress+ "http://localhost:3000/api/activitylog\" style=\"color:#336699;\">Back to Activity Log</a>");
            }
            else
            {
